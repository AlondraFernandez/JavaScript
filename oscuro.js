...
function mostrarProductos(productos = empanadas) {
    const productosDiv = document.getElementById('productos');
    productosDiv.innerHTML = '';
    productos.forEach(comida => {
        const productoDiv = crearElemento('div', 'card');
        productoDiv.innerHTML = `
            <img src="./assets/img/fondo-header${comida.id}.jpg" alt="${comida.nombre}">
...
function modificarProducto(id, campo, valor) {
    const producto = empanadas.find(r => r.id === id);
    if (producto) {
...
function eliminarProducto(id) {
    const index = empanadas.findIndex(r => r.id === id);
    if (index !== -1) {
...
function agregarCarrito(id) {
    const comida = empanadas.find(r => r.id === id);
    carrito.push(comida);
...
function filtrarProductos() {
    ...
    const productosFiltrados = empanadas.filter(comida => comida.nombre.toLowerCase().includes(filtro));
